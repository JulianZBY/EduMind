...
- 备课工作台采用主导航、会话二级侧栏与内容区布局，支持窄屏抽屉导航；生成模式位于会话输入框旁。
- 生成 PPT 使用 16:9 宽屏、封面/学习路径/内容卡片/总结版式，保留三套主题；长内容自动分到续页。
- 教案与试卷共用中文排版规范；答案解析单独起页。
- 「修改 PPT」接受 `.pptx`（20 MB 以内、1–80 页）及文字修改要求，返回独立文件和修改对照，原文件不覆盖。
- 上传编辑目前支持顶层文本框的标题和正文，保留图片及其他页面；不支持图片、表格、图表、组合图形、增删页或布局重设计。修改段落沿用首个文本片段的样式，复杂混排及长文本需下载检查。
- 需配置真实的 `LLM_PROVIDER` 与对应 API Key；stub 模式明确返回不可用。

在 `backend` 目录运行 `python -m scripts.design_review_sample` 可生成样式样例。增加 `--live-edit --env-file <配置文件路径>` 可执行真实模型编辑验证（会调用已配置的模型服务）。样例位于 `artifacts/design-review/`。
