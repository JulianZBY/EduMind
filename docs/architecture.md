...
- 已实现：备课对话（无状态版）、文档上传与后台解析、知识库语义检索、图谱数据读取、试卷 / 互动内容生成、
  课件与教案修改、冲突列表与三选一裁决、生成物文件下载。
- 未实现（按票号）：会话与生成物持久化（票 05/07）、能力注册统一（票 02/03）、前端六区骨架（票 04/06/08~13）、
  题库查询（票 12）、设置界面（票 13）、结构冲突与常识存疑的检测逻辑（ADR-0004 只落模型与动作）。
